...
def generate_random_address():
    ...
    # Use range index for city to correspond with state and zip code
...
def write_row(user_id, product, date, address):
    price = products[product][0]
    quantity_ordered = numpy.random.geometric(
        p=1.0-(1.0/price), size=1)[0]  # Low price High quantity orders
    output = [user_id, product, quantity_ordered, price, date, address]
    return output
...
if __name__ == '__main__':
    ...
    for month_value in range(1, 13):
        ...
        if month_value == 11:
            orders_amount = int(numpy.random.normal(
                loc=20000, scale=3000))  # Slightly high sales

        if month_value == 12:
            orders_amount = int(numpy.random.normal(
                loc=26000, scale=3000))  # Highest sales
        ...
        while orders_amount > 0:
            ...
            df.loc[i] = write_row(user_id, product, date, address)
            i += 1

            if product == "iPhone":
                if random.random() < 0.15:
                    df.loc[i] = write_row(
                        user_id, "Lightning Charging Cable", date, address)
                    i += 1
                if random.random() < 0.05:
                    df.loc[i] = write_row(
                        user_id, "Apple Airpods Headphones", date, address)
                    i += 1
                if random.random() < 0.07:
                    df.loc[i] = write_row(
                        user_id, "Wired Headphones", date, address)
                    i += 1

            elif product == "Google Phone" or product == "Vareebadd Phone":
                if random.random() < 0.18:
                    df.loc[i] = write_row(
                        user_id, "USB-C Charging Cable", date, address)
                    i += 1
                if random.random() < 0.04:
                    df.loc[i] = write_row(
                        user_id, "Bose SoundSport Headphones", date, address)
                    i += 1
                if random.random() < 0.07:
                    df.loc[i] = write_row(
                        user_id, "Wired Headphones", date, address)
            ...
            if random.random() <= 0.02:
                ...
                product_removed_list = [
                    product_2 for product_2 in products_copied]
                weights_2 = [products_copied[product_2][1]
                             for product_2 in products_copied]

                product2 = random.choices(
                    product_removed_list, weights=weights_2)[0]
                df.loc[i] = write_row(user_id, product2, date, address)
                i += 1
